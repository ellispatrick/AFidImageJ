...
public class AutofluorescenceRemover implements PlugIn {
    ...
    public void run(String arg) {
        ...
        GenericDialog gd = new GenericDialog("AFid");
        ...
        gd.addCheckbox("Intersection Mask Only", false);
        gd.addNumericField("Correlation Cutoff (0 if not used)", 0.60, 2);
        gd.addNumericField("Number of clusters (1 if not used)", 1, 0);
        gd.addNumericField("Max Value to automate k (0 if not used)", 0, 0);
        gd.addCheckbox("Dilation", true);
        // gd.addCheckbox("Extended Center Finder", false);
        gd.addNumericField("Dilation Sensitivity:", 20, 0);
        gd.showDialog();
        if (gd.wasCanceled()) {
            return;
        }
        ...
        boolean andOnly = gd.getNextBoolean();
        ...
        // imp1ThresholdFiltered.show();
        // imp2ThresholdFiltered.show();
        ...
        impAndFiltered.setTitle("Intersection Mask");
        impAndFiltered.show();
        if (andOnly) {
            return;
        }
        ...
        IJ.log(String.valueOf(andBX.length));
        ...
        summaryTable.addValue("AF Count", Double.NaN);
        summaryTable.addValue("k Chosen", Double.NaN);
        summaryTable.addValue("T Statistic", Double.NaN);
        ...
        for (int i = 0; i < lengthAnd; i++) {
            ...
            for (int y = 0; y < roiHeight; y++) {
                for (int x = 0; x < roiWidth; x++) {
                    ...
                    if (ipAndFiltered.get(roiX + x, roiY + y) == (i + 1) || ipAndFiltered.get(roiX + x, roiY + y) == ipAndFiltered.getMax()) {
        ...
        if (kAuto == 0) {
            ...
            double[] kVals = new double[kAuto-2];
            ...
            double x1 = idxMax+3;
            double x2 = kAuto;
            double y1 = statVals[idxMax];
            double y2 = statVals[kAuto-3];

            double[] xLine = new double[] {x1,x2};
            double[] yLine = new double[] {y1,y2};

            // Output Plot
            Plot kPlot = new Plot("Statistic Value", "k", "Statistic");
            kPlot.add("circle", kVals, statVals);
            kPlot.add("line", xLine, yLine);
            kPlot.draw();
            ImagePlus kPlotIm = kPlot.getImagePlus();
            kPlotIm.show();
            ...
            for (int i = idxMax; i < statVals.length; i++) {
                double x0 = i+3;
                ...
                if (dist > maxDist) {
                    ...
                    summaryTable.setValue("k Chosen", 0, (double) bestK);
                    summaryTable.setValue("T Statistic", 0, y0);
                }
            }
        ...
        summaryTable.addValue("AF Count", (double) totalAF);

        summaryTable.show("AFid Summary Table");
...
//            }
        ...
        if (!glowRemove) {
            ...
            imp1Glow.setTitle("Dilation 1");
            imp1Glow.show();
            imp2Glow.setTitle("Dilation 2");
            imp2Glow.show();
